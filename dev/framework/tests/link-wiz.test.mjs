...
const viewerContainer=new E('div',doc);let viewed=null;const vr=wiz.render(viewerContainer,{type:'media',href:'/x.pdf',target:'viewer',label:'PDF'},{navigate:e=>viewed=e});assert.equal(vr.node.tagName,'SPAN');vr.node.dispatch('click',{preventDefault(){}});assert.equal(viewed.target,'viewer');assert.equal(viewed.href,'/x.pdf');
